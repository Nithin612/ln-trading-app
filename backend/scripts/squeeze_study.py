...
def main() -> None:  # noqa: C901 - one linear pass: load, benchmark, accumulate, render
    ...
    for sym, df in frames.items():
        ...
        # weekly analogue of Carter's higher-timeframe filter: 5x the lengths
        ma_w = cs.rolling(_BB_LEN * 5).mean().to_numpy()
    ...
    a(
        f"_Generated {datetime.now(tz=UTC).date()} - {len(prepared)} liquid stocks - CA-clean "
        f"window from {_CLEAN_SINCE.date()} - {n_fire:,} squeeze fires "
        f"({n_long / n_fire * 100:.0f}% long)._\n"
    )
    a(
        ...
        "**t is on the daily mean of the excess** (n = trading days), never per trade.\n"
    )
    order = [
        "squeeze fire (all)",
        "squeeze fire, long",
    ...
    for label in order:
        ...
        for k in _HORIZONS:
            ...
            a(f"| {label} | +{k}d | {total:,} | {mean_pct:+.3f}% | {tstat:+.2f} ({ndays}d) |")
    a(
