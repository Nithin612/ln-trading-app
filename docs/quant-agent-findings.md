...
| **A21** ✅ **DONE 2026-09-05** | Mark-to-bid, so marks match fills. Shipped as `exit_mark()` routing marks through `simulate_fill` with the EXIT side; wired into `update_position_pnl` (API list, summary, monitor — batched depth) and `_open_book_mtm`. ⚠ One limit: the HISTORICAL mark cannot be true mark-to-bid (depth is Redis-only, 60s TTL, never persisted; using today's book for a past cutoff would be real look-ahead). The review pass also found `_round_tick` could round an off-grid price back PAST its reference (4.9% of real closes) — now directional, which fixes the entry-fill path's stated contract too. | 0.5 d |
| **A37 + T3** ✅ **DONE 2026-09-05** | Volume-participation cap on fills, with its parametrised test. Shipped as `participation_bps` (`k × participation²`, zipline calibration) on BOTH fill paths + every mark surface (A31). Real book: ADROITINFO is **16.95% of daily volume** on ₹33k, now 28.7 bps vs the old 2 bps floor; only 3 of 29 positions charged >1 bp. ⚠ NOT a partial-fill cap (needs partial fills = Phase 7) so no signal is suppressed; ⚠ backtest untouched (FROZEN engine). | 1 d |
| **A29** ✅ **DONE 2026-09-05** | Flat DP charge per delivery sell + per-trade cost floor. ₹15.34 flat on the delivery SELL leg, size-independent. **Measured: all 105 closed positions were delivery, so ₹1,610.70 was never levied — 15.8% of the book's whole loss; realised should read −₹11,829 not −₹10,218.** A small position now pays 61.6 bps of charges vs 22.4 for a large one. The cost FLOOR ships as a mechanism defaulting to 0 — the Zerodha schedule has no minimum and inventing one would fabricate a cost. | 0.25 d |
| **A23** ✅ **DONE 2026-09-05** | Date-versioned (effective-dated) fee registry. `SCHEDULE_HISTORY` + `schedule_for(on)`; each LEG costed on its own date (`entry_on`/`exit_on`), `priced_on` recorded in the breakdown. A pre-coverage date RAISES rather than guessing. Ships with ONE entry on purpose — we hold no researched rate history and inventing effective dates would fabricate precision. ⚠ **Found while wiring: `app/backtest/` models NO costs at all**, so backtest P&L is GROSS vs paper NET — frozen-engine, recorded not fixed. | 0.5 d |
| **A26** ✅ **DONE 2026-09-05** | Refuse loudly at the hot-set capacity boundary. The cap is now HARD for discovery tiers and SOFT for protected ones: a signal/trigger-bound stock is NEVER dropped (dropping one removes a signal from the record), and an overflow escalates at ERROR with numbers + remedy and rides `provisional:health:{day}` so it cannot hide in a log line. Refusing to RUN was rejected — ours is a self-imposed CPU budget, so refusing would score nothing. Rule extracted to a pure `apply_hotset_cap`. | 0.5 d |
| **A25** ✅ **DONE 2026-09-05** | Assert the tick mode on the depth path. Both live paths (`live_worker`, the dormant v1 `tick_consumer`) now census each batch's `mode`: `degraded` = a non-full tick on our MODE_FULL subscription (the documented Kite downgrade — no book, so `depth:{stock_id}` goes stale and spread-aware fills fall back to the flat floor, i.e. **paper fills quietly cheaper than reality**), `depth_missing` = a *tradable* full-mode tick with an unusable book (the symptom the mode counters cannot explain; index packets excluded — full mode, bookless by design). A mode-less tick counts as `unknown` and never alarms, or replay would cry wolf every run. Surfaces: heartbeat stats · a rate-limited warning naming counts + consequence + remedy · a durable `tickmode:health:{day}` (7-day TTL) rendered by the daily report beside the 6.8.6 alarm. **Deliberately does NOT reopen the socket** as repo 8 does — never observed against us, and a reconnect loop on a misread costs more than the degradation. Clean feed = zero extra Redis round trips. | 0.25 d |
| **H6** ✅ **DONE 2026-09-05** | Degenerate-ratio hygiene. `app/core/ratios.py` is now the single definition of the contract: **UNDEFINED is `None`, never `0.0`** (a zero-risk signal used to return the same number the code prints for the WORST possible reward:risk, so "not assessable" and "terrible" were indistinguishable); **OFF-SCALE is clamped AND MARKED** (`>50`, since a truncated 228 printed as "50.00" reads as a real 50:1 setup — worse than the artifact). **`MAX_RR = 50` was read off the book, not argued**: across all 656 signals with levels, R:R p50 1.97 · p90 3.67 · **p99 28.5** · max 228.06, and **exactly one row (0.15%) exceeds 50** — the tiny-SL artifact whose stop is **2.6 bps** of its price. ⭐ **Rule: clamp what you REPORT, never what you DECIDE** (every gate decides on the raw ratio; `MAX_RR` sits above every threshold that reads it, and the ordering is pinned by test). Four literals doing THREE jobs in four modules, two disagreeing by 1000×, consolidated without collapsing them. | 1 h |
...
| Item | What | Effort |
|---|---|---|
| **H8** ✅ **DONE 2026-09-04** | Noise negative-control — prove the DSR bar rejects. **The bar is SOUND**: 1.10% of best-of-20 zero-edge selections clear against a 5% allowance, and 80% power at a true Sharpe of 0.52. ⭐ Restated, it demands **t ≈ 3.6, flat in n**. `dsr_control.py` + 14 tests. ⚠ The spec was insufficient — "does it reject noise" is passed trivially by a bar that rejects everything; the power arm is what made the verdict readable. | 0.5 d |
| **H1** ✅ **DONE 2026-09-05** | Moving-block bootstrap beside PSR/DSR/MinTRL. Künsch moving blocks (`L=ceil(n^(1/3))`) so short-range dependence survives resampling — DSR's own stated weakness is the independence assumption. Validated H8-style: **6.2% false-positive at n=44 against a one-sided 5% design, 95% power at a true Sharpe of 0.5**. ⭐ **First read: the book's Sharpe is −0.033 with a 90% interval [−0.223, +0.118] — at n=105 even the LOSS is not established, so any gate partitioning this series is partitioning noise.** Blocks earn their keep: 9% wider than iid on the real book, **77% wider at φ=0.8** on synthetic AR(1). ⚠ **A defect was found IN the instrument while validating it**: a near-constant series with one outlier produced 76% zero-variance resamples that were silently dropped, leaving only the draws CONTAINING the outlier — it now refuses above a 10% degenerate share. ⚠ Block order had to become a property of the data (`Row.at`), since sidecars sort rows for display. | 0.5 d |
| **H12** ✅ **DONE 2026-09-05** | Beta-to-NIFTY + information ratio on every cohort. `app/services/beta_ir.py` → daily report (closed book + LONG/SHORT split). **First read: beta +0.638 overall (LONG +0.34, SHORT +1.49)** against a +0.17 median across 4,843 replications — market exposure far above the published median. | 1 d |
| **H2** ✅ **DONE 2026-09-05** | Buy-and-hold benchmark line in the daily report. `app/services/buy_and_hold.py`. **First read: NIFTY 50 −1.61% vs the book −17.52%** over 2026-08-17→09-04 — a 15.92 pp shortfall. The partial-deployment caveat is **direction-aware**: the index FELL, so under-deployment makes the comparison worse, not better. | 0.5 d |
| **H11** ✅ **DONE 2026-09-05** | MinTRL as the banner headline — the DSR line now leads with `n=120 of ≈715 needed`, and says **"MORE DATA CANNOT RESCUE IT"** when the observed Sharpe does not exceed its benchmark (the case where "keep accruing" is not merely unhelpful but actively wrong). The sidecars' `n/20` bar is now labelled a PROCESS convention beside it — `sl_atr` passed all three readiness guards at **t ≈ 0.41 against a 3.6 hurdle** while its 17/20 read like progress. | 2 h |
| **T11** ✅ **DONE 2026-09-05** | Pin PSR/DSR against independently derived values. ⭐ Pinned as a **VERDICT FLIP**, not a rounding difference: at **n=10, SR=0.585 the correct PSR is 0.9476 (FAILS a 95% bar) while the QuantStats excess-kurtosis convention reports 0.9668 (CLEARS it)** — same data, opposite decision. A first draft asserting merely "excess gives a bigger number" passed happily in the saturated region where both round to 1.0 and nothing is at stake. Also pins the `moments → psr` **seam** (the only path the bug can travel), E[max SR] against the Euler–Mascheroni form, MinTRL, and end-to-end DSR on a fixed series. | 2 h |
| **H4** ✅ **DONE 2026-09-05** | Gate/hypothesis register as data (absorbs **A8**, the failure archive) — `app/services/gate_register.py`: 17 hypotheses carrying prediction · bar · stands-at · verdict · review-due, transcribed from the PHASES REVIEW CALENDAR. A Python module rather than YAML so it is type-checked and importable, with a contract test mapping every `*_gate_mode` knob to an entry — a knob shipped without one is exactly how a trial escapes the count. Both reversals stay visible on purpose: a programme that forgets its failures re-runs them. | 1 d |
| **U4** ✅ **DONE 2026-09-05** | Trials-attempted counter — **observed 15 vs 20 assumed**, rendered in the daily report beside the bar it feeds. ⚠ **The counter states that this does NOT license calling the bar conservative**: one entry is one HYPOTHESIS, most were tried at several thresholds (`sl_atr`'s k, chase's 0.33R, circuit's 1.5%, liquidity's floor), and each variant is its own trial — so the observed count is a LOWER BOUND and variants are not recorded yet. `entry_diversity` and the notional cap are excluded with reasons (a mandated rule and a safety rail claim no edge). Failures COUNT — dropping them is the exact selection bias the deflation corrects. **`DEFAULT_TRIALS` untouched**: raising the assumed N makes the bar harder for everything and is a decision for a person. | 0.5 d |
| **H3** ✅ **DONE 2026-09-05** | VIX companion as a trailing percentile (shadow; changes no trade) — ⭐ **the inherited absolute `20` turns out to be India's 94.8th percentile**: median VIX **13.35** across 784 sessions, above 20 on only **5.1%** of days, so it reads as "somewhat nervous" and behaves as an extreme (p80 = 15.73). Now self-calibrating, with the absolute as a shallow-history fallback and `vix_basis` naming which produced the flag. ⚠ Also corrected the STALE claim that made the number un-revisitable — the module said VIX history was "too shallow (~weeks)"; it holds **784 sessions**, the backfill having happened like the index one. VIX still never blocks. | 1 d |
| **T7** ✅ **DONE 2026-09-05** | Exhaustive-enum mapping tests. **The gate-mode vocabulary was declared NINE times and tied together nowhere** — nine `*_gate_mode` settings each say `Literal["off","shadow","active"]` independently while `_effective_mode` handles them with no link back, so a fourth mode added to one knob falls through as "off" silently, on the order path. `GATE_MODES` is now the single declaration and a contract test introspects the settings model against it. Also pins `_effective_mode` over every ordered PAIR (where the `"off"`-is-truthy bug lived), every `EnforcedBy` variant, and the registry order as a golden list. ⚠ **One draft test was rewritten because it passed VACUOUSLY** — "broker rules come last" is what the registry's comment says but not what it declares (`offmarket` is a BROKER rule declared FIRST, by design). | 2 h |
| **A24** ✅ **DONE 2026-09-05** | Standing rule: never render a precise figure without its uncertainty — shipped into `.claude/rules/ui.md` (no bare point estimate for anything predictive; a projection must depend on what it projects; round to the precision the evidence supports; a sample size travels with its statistic; "not assessable" is a legitimate rendering). | 1 h |
...
**▶▶ BUCKET B IS COMPLETE (all 11 items), as is BUCKET A (all 8).** Verified 2026-09-06 against
the artifacts rather than the checkboxes — every item has code **and** tests on disk. This table
had drifted: H8, H12, H2 and A24 showed no ✅ here while the harvest queue below recorded them
DONE. Fixed on discovery, per working rule **W1**. Remaining pre-cycle-2 work and the five open
decisions are queued in `docs/phases/pre-cycle2-queue.md` on branch `feature/pre-cycle2-hardening`.
...
| **H2** ✅ **DONE 2026-09-05** | **Buy-and-hold benchmark line in the daily report** — ⭐ **first read: NIFTY 50 −1.61% vs the book −17.52% over 2026-08-17→09-04, a 15.92 pp shortfall.** The partial-deployment caveat is **direction-aware** and here it makes things WORSE, not better: the index fell and the book fell ten times further on ~45% of capital at risk. A fixed "we were under-deployed" sentence would be exactly the misleading line the report exists to prevent (the `tail_guard` lesson). | `app/services/buy_and_hold.py` | ~half day | We do not have one — I checked; `benchmark.py` is per-signal relative strength, not a portfolio baseline. AgentQuant's most sobering number was buy-and-hold beating five years of agent work, and it was invisible until someone opened a CSV. Our book is **−0.303R expectancy**; the honest denominator is what NIFTY did over the same window. Cheap, and it reframes every subsequent gate argument. |
...
| H6 ✅ **DONE 2026-09-05** | `MAX_RATIO` sentinel instead of `inf` for degenerate ratios | `app/core/ratios.py` + 8 call sites | ~1 hour | We have the `RR≈228` tiny-SL artifact on record. Trivial hygiene. **Shipped — and it was not only hygiene:** `_reward_risk` is a **dedup tiebreaker**, so an uncapped 228 let a 4-paise stop beat every genuine candidate for the row actually shown; and `daily_report` divided by `capital_inr` **unguarded in one place and guarded in another, thirty lines apart, same expression**. |
| **H6-b** ⏳ **DEFERRED — needs §8 + sign-off** | The frozen ratio sentinels H6 could not touch | `app/backtest/engine.py` (FROZEN), `app/backtest/metrics.py` (golden-pinned) | ~half day + regression | Three, found by the H6 sweep. (1) **`_compute_sortino` returns `0.0` when there are NO losing trades** — a genuinely infinite Sortino reported as the WORST possible score, and it is a §8 golden field persisted to `Numeric(6,3)`. (2) `_compute_sharpe` guards `std == 0` by **exact equality**, so a `1e-16` float-noise stdev yields a Sharpe of ~`1e15` into that same column — a DB numeric-overflow before anyone sees a wrong number. (3) `metrics.avg_rr` is uncapped into `Numeric(5,2)` (max 999.99). Each changes a golden, so each needs the §8 regression + regenerated Rust fixtures, per the engine freeze. |
...
| **A24** ✅ **DONE 2026-09-05** | **Never render a precise figure without its uncertainty** — written into `.claude/rules/ui.md` as a section ui-reviewer enforces. Six clauses: no bare point estimate for anything predictive; **a projection must depend on what it projects** (their "probable exit date" did not depend on the target price it was the date for); round to the precision the evidence supports, not the float; **a sample size travels with its statistic** (H11); "not assessable" is a legitimate rendering and beats a plausible default (H6's undefined / off-scale / measured must stay visually distinct); and **a caveat must branch on the data** — a fixed hedge that is wrong in some branch is worse than none (`buy_and_hold._deployment_note`). | ~1 hour to write down | *(repo 6C)* Their "probable exit date" is a calendar date with no volatility term that **does not even depend on the target price it is the date for**. A precise number reads as a confidence signal. Same failure as repo 3's vote-share-as-confidence and repo 1's placeholder-as-metric — and we are specifically exposed, because a 2–3%/day goal invites converting a wish into a timeline. |
...
| # | Item | Where it lands | Effort | Why now |
|---|---|---|---|---|
| **W1** ✅ **DONE 2026-09-06** | **Make doc/code precedence an explicit rule** — "if a doc disagrees with the code, the executable content wins; fix the doc in the same change" | CLAUDE.md | ~15 min | *(repo 9)* We hold this as a habit ("trust the artifact over the checkbox") but state it as a value, not a precedence rule. A rule tells a future session **what to do** on finding a conflict, not merely that conflicts are bad. |
| **W2** ✅ **DONE 2026-09-06** | **"Do not add parallel implementations"** as a written rule | CLAUDE.md / rules | ~15 min | *(repo 9)* **The rule with the most evidence behind it for us**: our review round found **five separate Buy surfaces**, one unwired, and eligibility gating had to be retrofitted across all of them. |
...
| W3 ✅ **DONE 2026-09-06** | **Same-commit config hygiene** — a new config item updates `.env.example` and its docs in the same change | rules | ~15 min | *(repo 9)* The narrow checkable instance of our doc-sync ritual. Config drift is what bit us when a `.env` gate flip did not reach a running process. |
| W4 ✅ **DONE 2026-09-06** | **Decide the git boundary deliberately** — theirs forbids `commit`/`tag`/`push` without confirmation; ours reserves push only | CLAUDE.md | ~15 min | *(repo 9)* Worth writing down rather than inferring. The current working pattern (commit freely to a worktree branch, hold pushes) is fine — but it should be stated. |
| W5 ✅ **DONE 2026-09-06** | **Forbid hardcoded model names alongside secrets, paths and ports** | rules | ~15 min | *(repo 9)* The non-obvious clause. Our analogue: `STATUS.html` hardcodes gate modes in prose and an ASCII diagram, so a mode flip silently falsifies it — a config value copied somewhere that cannot track it. |
