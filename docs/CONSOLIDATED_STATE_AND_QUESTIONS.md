...
| **2** | Wire the ledger + a backend wiring lint | ✅ **BUILT 2026-09-18** — `app/broker/ledger_wiring.py` (`record_fill` on every fill, `record_close` on every close, `chain_id` derived via `uuid5` so entry+exit reconstruct as one trade with no schema change) + `tests/test_backend_wiring.py`, the backend half of `apiWiring.test.ts`. **306 money-path tests pass.** ⭐ **Fails CLOSED in the fill's own transaction**, deliberately against the overlay pattern: an overlay failing open declines to interfere, a ledger failing open lets money move unrecorded. ⛔⛔ **bug-hunter found 7 confirmed defects, all fixed** — worst: `LedgerError` subclassed `ValueError` and both order callers catch `(PaperOrderError, ValueError)` then **COMMIT**, so a ledger failure would have committed the fill, returned 422 and left a position with no row — **fail-closed defeated by a base class**. Also: `as_of` used the UTC date on a column documented as the MARKET date (00:00–05:30 IST filed to the previous session); `realized_pnl` recorded unrounded against `Numeric(14,2)` so the ledger could never reconcile with the book; `data_version` changed daily, duplicating `as_of`. ⭐⭐ **And the lint itself was satisfied by a COMMENT** — `\brecord\(` matched `` `record()` refuses a blank one `` in my own module, so deleting both call sites would have left it green. Now parses `ast.Call`, with a canary that strips the call sites and asserts it fails. **Third vacuous test caught this session.** |
...
| **5** | Re-run E2 | ✅ **3a DONE 2026-09-19** — `e2_score_ic.py --pit`, 31,378 panels / 156 sessions, PIT cohort via `liquid_as_of`. **IC −0.0055, SE 0.0084, t −0.65, 90% [−0.0192, +0.0083] ⇒ NULL**, break-even 0.0313 (upper bound **3.8× below** it). ⭐⭐ **THE LOOK-AHEAD WAS REAL AND DID NOT DRIVE THE CONCLUSION** — repairing M62's cohort leaves the verdict unchanged and the interval **TIGHTER** (0.0378 → 0.0275). ⭐ **CA-robustness prediction HELD**: 3a is a Spearman RANK correlation, so dropping every CA-tainted row moves the IC by **+0.0001**. ⭐ Constants measured clean: `sd(IC_t)` **0.1046** (assumed 0.10) · `E[z\|sel]` **1.8758** (assumed 2.268) · σ_cs **4.340%**. ⛔⛔ **ITEM 19's RETIRE BRANCH FIRES** (−0.0055 < 0.0121), on the first branch not the ambiguous middle; **the holdouts stay sealed** (they open only at ≥0.0499). ⚠ **3b INCONCLUSIVE and REPORTED-NOT-DECIDED** — it is a MEAN contrast and so, unlike 3a, not CA-robust ⇒ **needs item 16, and the ≥70% GATE stays unproven both ways.** ⛔ **Item 21 was NOT a blocker** (measured: costs must fall below the 22.22 bps statutory floor to matter) and the three named CAs are all in **holdout-1**, none in the test block. ~~Needs **14** and **21**.~~ ⭐ **No longer blocked on 16 (Grok):** exclude the three named corporate actions by dated list — IRCTC 1:5, BAJAJFINSV 1:1 bonus + 1:5, PEL demerger. Window **797, DECIDED**. Arms 5b (per-direction) and 5c (bar as the formula `break-even + 1.645·SE`, per arm) |
| **6** | Re-run D5 + D1 + B7 | ✅ **DONE 2026-09-19** — `scripts/item6_rerun.py` + `scripts/item6_b7.py`; reports `item6-rerun-2026-09-18.md`, `item6-b7-2026-09-19.md`. Dependence characterised FIRST as instructed: it is a GROUPING (trades sharing a session), not a lag ⇒ cluster-robust by entry session, and a single indicator regression for B7's contrast because both arms sit on the same days. New estimators in `block_bootstrap.py` (`intraclass_correlation` · `design_effect` · `cluster_robust_mean_t` · `cluster_robust_slope_t`), **validated against a known null that first REPRODUCES the failure** — naive rejects 36.7% at a nominal 5%, clustered 5.0%. ⭐⭐ **THE BIGGEST FINDING IS REPRODUCIBILITY, NOT THE TREATMENT: `_load_frames` has no cohort, it has a QUERY whose answer moves.** `is_active` was 1,322 when D5/D1 published (09-08) and is **2,292** after D2′b (09-14) ⇒ corpus **1,152 → 3,267** with nothing in the study changed; **the published |t| ≤ 0.65 and −0.026R cannot be rebuilt.** ⇒ *a cohort defined by a live query against mutable state is a timestamp, not a cohort — PIN the name list (`liquid_as_of`), never re-derive it.* ⭐ **Delete treatment: 0 of 3,267 (D5) · 0 of 2,029 (D1) · 4 of 350 (B7).** The zeros were VERIFIED not assumed (predicate fires on a constructed record; real min fill-to-stop margin **+0.12%**, median **+5.000%**) ⇒ a genuine no-op on wide-stop daily corpora. ⭐⭐ **B7 gave M64 its first REAL-DATA demonstration: 4 trades (HFCL · MUTHOOTFIN · TCS · WIPRO), every fill on the wrong side of its own stop, every one T=0, every one booking EXACTLY +1.0000R**, all with stop widths below the 2% notional-cap floor. ⛔ **And the standing caveat is half wrong** — refusing them makes the T=0 contrast STRONGER (−0.664 → −0.824, t −4.73 → −6.61), not weaker. **F9: every headline survives both corrections** — no sign change, no |t| = 1.96 crossing. D5's verdict is unchanged and sharper (every constant-R:R target still worse, t −2.5…−3.7); D1 §1 stays flat across RVOL buckets (|t| ≤ 0.60); B7's MFE/\|MAE\| ≈ 1.02–1.08 ⇒ still no geometry repair |
...
| **11** | Holdout seal — **617 sessions**, whitelist = row/session counts + integrity checks | ✅ **SEALED 2026-09-19** — `scripts/holdout_seal.py`, per-session + per-block digests, 9 tests. ⛔ **It stays shut: item 5's 3a returned −0.0055, so item 19's RETIRE branch fired and the ≥0.0499 opening condition did not.** Verified 617 sessions / 1,053,245 bars |
| **11b** | ⭐ **Holdout-2 seal: 313 sessions (2019-10-01 → 2020-12-31)** | ✅ **SEALED 2026-09-19** — 313 sessions / 466,732 bars. ⭐⭐ **Sealing it found a quoted fact to be wrong: `313 + 617 + 797 = 1,727` — the test block is 798.** The holdouts are CLOSED intervals and immutable; the test block gains a row every session, so **797 was a timestamp, not a design property**. Blocks are now pinned by DATE RANGE with counts as outputs. The three partition the archive exactly, 1,728 of 1,728 |

### Ready now — no decision, no research risk

| # | Item | Note |
|---|---|---|
| **2** | Wire the ledger + a backend wiring lint | ✅ **BUILT 2026-09-18** — `ledger_wiring.py` + `test_backend_wiring.py`. **Verified writing real production rows**: 4 `position_lifecycle` entries from the four shorts closing, each carrying full provenance |
...
| **16** | CA policy — the external NSE source (D3's free `/api/` path) | ✅ **DONE 2026-09-19** — `app/services/nse_corporate_actions.py` + `scripts/backfill_corporate_actions.py`, 22 tests. ⭐⭐ **A CACHE of the authority's answers, replacing a RULE we evaluate** — the 25% gap screen is 62.5% false-positive (M70). ⭐ The `corporate_actions` table already existed with the right shape and **0 rows**; its only writer was the admin-verified manual path, so what was missing was the FETCH. ⛔⛔ **Two conventions that silently corrupt an adjustment, both mutation-pinned: NSE publishes the BONUS ratio while the model stores the TOTAL** (`Bonus 3:1` = 3 new per 1 held ⇒ **1:4**, not 1:3) **and a face-value split moves OPPOSITE to the share count** (₹10→₹1 is a *tenth* of the face value ⇒ **ten times** the shares). ⚠ The ratio is published as **prose, not a field**, so a parse is unavoidable — handled by storing the raw subject verbatim on every row and REPORTING what could not be parsed rather than dropping it. A manual row is never overwritten; a manual/NSE disagreement is surfaced. **Live-verified Jan 2024: 9 parsed / 34 unsupported-and-visible, and NESTLEIND 2024-01-05 → split 1:10 — the authority independently confirming the event the gap screen flagged.** ⛔ **This is what item 5's 3b needs**; the backfill itself writes to the DB and awaits the user |
| **17** | Measure spread and impact | Falsifier: median half-spread on the tradability-screened subset < 5 bps ⇒ not binding |
| **18** | Index/VIX for 2021–22 | ⏸ **DEFERRED BY THE USER 2026-09-19, with a TRIGGER: raise it again when item 5's 3b is reached.** ⛔⛔ **Its stated justification did not survive the seals** — the queue's reason is *"the market-regime overlay is unevaluable across the holdout-1 era"*, and evaluating an overlay there **is opening the holdout** (the seal forbids "any model fit, threshold choice, factor selection or hyper-parameter tuned on them"). The item predates the seal. ⭐ **The enabling bugfix IS shipped:** `backfill_indices` filtered `weekday() < 5`, making NSE weekend sessions structurally unreachable — the same defect `backfill_ohlcv_history` had already fixed. Verified live: Saturday 2024-03-02 returns **110 lines**. ⭐ **Measured, so the decision is costed:** index+VIX hold **793 sessions, ALL in the test block, ZERO in either holdout**; the archive DOES serve 2020/2021/2022 (probed); the backfill is **1,371 calendar days ≈ 23 min**, insert-only and idempotent. ⚠ **Honest qualification: the holdout era is already partly characterised in our own docs** (COVID +38.1%/yr · 2021 +34.7% · 2022 −0.1%/yr, 42.7% down-days), measured from the cohort tape — so the blocks were never pristine, though the seal's guarantee (nothing changes before opening) still holds |
| **20** | Tax in the cost model | ✅ **DONE 2026-09-19** — `app/trading/tax.py`, 14 tests. **The correction is asserted as the first test: tax is zero at break-even, so break-even does not move** — adding it to the cost stack would have made every break-even in the programme, item 5's included, too pessimistic. ⭐⭐ **The Budget-2024 change (2024-07-23: §111A 15%→20%, §112A 10%→12.5%, exemption ₹1L→₹1.25L) sits INSIDE the 2023-07-03→ test block**, so both regimes are in the window and a single-rate model would be wrong for ~its first year — hence A23-style effective dating. ⛔ **Intraday returns `SLAB_UNKNOWN`, never a guessed rate** (speculative business income is taxed at slab with all other income), and intraday is decided by PRODUCT not by the dates. ⚠ The ₹1.25L exemption is ANNUAL and shared, so priced in isolation the second long-term disposal of a year is under-taxed — `long_term_exemption_used` threads it |
| **22** | Version the gate configuration | ✅ **DONE 2026-09-19** — `gate_config_versions` (migration `d4e5f6a7b8c9`, **not yet run on dev**) + `app/services/gate_config_history.py`, 10 tests. ⭐⭐ **The snapshot is MORE than the modes, and that is the design point: a modes-only version cannot show that a RULE DID NOT EXIST YET** — `settlement` and `universe_membership` were both added after signals started being minted. Taken from `REGISTRY` **and** `RestrictionConfig` (W2 — the composed objects the order path walks, not a hand-maintained list). Append-only, deduped by content hash, so the table is the history of CHANGES not a log of runs. ⛔ Decimals serialise as **strings** — a float round-trip would let two machines with identical config hash differently and emit spurious changes. ⚠ Attribution is **by time, not by FK**: a column on `signals` would need a backfill that is *invented* for every existing row, and `version_as_of` returns **None** before the history begins rather than today's config — the exact fabrication the table prevents |
| **23** | The three standing concessions, enumerated | ✅ **DONE 2026-09-19** — `docs/analysis/standing-concessions-2026-09-19.md`. Each stated as *what is conceded · why · what it costs, MEASURED · what would close it*. ⭐ **Measured while writing it: `entry_diversity` refuses 7 of 52 live signals (13.5%), and the single-factor-dominance half of the gate has fired ZERO times** — the `<2 factors` clause does all the work. ⚠ The two portfolio rails (6% heat, max-3 positions) were chosen from **friction arithmetic** (M77), not from any portfolio evaluation. ⚠ **Concession 3 is NOT item 21** — 21 reconciles *charges* (a published schedule), this is the *fill price*, which no schedule can settle |
| **24** | Position-count cap | ✅ **DONE 2026-09-19** — wording corrected in `config.py` and `risk_engine.py`, 4 tests pinning the numbers to `fees.roundtrip_charges` (W5). Ladder re-measured, reproducing M77 to ~1%: **1→23.76 · 3→26.61 · 5→29.89 · 6→31.21 bps**, crossing 30 between 5 and 6. ⭐⭐ **And the mechanism is NOT risk — it is the FLAT ₹15.34 DP charge per delivery sell**, so splitting capital multiplies a fixed cost; concentration control is the side-effect, not the justification. **Tested, not asserted:** stripping the flat component leaves a ladder spanning <1 bps across 1–8 positions. ⚠ At larger capital the ladder shifts down and 3 stops being right |
...
| **27** | **F-1 — `StockDetailPage` paints the LTP `--color-bull` regardless of direction** | ✅ **FIXED 2026-09-19.** ⚠ The live feed could not supply the fix: `LtpQuote` is `{symbol, ltp, ts}` with **no reference price**, so the basis is the last COMPLETED daily close (today's own bar excluded, or the change collapses toward zero through the session). ⭐ **THREE states** — up / down / **not assessable**, neutral rather than defaulted to green (A24). Direction is **glyph + colour** and the **basis is rendered** (`−5.00% vs prev close`) so the colour can be checked rather than trusted. ⚠ The green `TrendingUp` beside *Live* was fixed too: liveness is not direction. 6 tests, mutation-verified |
| **28** | **B-4 — the FII/DII consumer states "flows neutral" from zero rows** | ✅ **FIXED 2026-09-19.** **Measured first: 30 of 52 signals carried the string while `fii_dii_daily` held 14 rows.** ⭐ **The frozen engine is NOT the bug and was NOT touched** — it is handed `Decimal("0")` and correctly describes zero; the lie is manufactured in `get_market_flow_5d`, which resolved a missing row to zero. Now returns **`FlowWindow`** with `sessions_with_data` (`.is_absent` / `.is_partial`), **still unpacking as a 2-tuple** so all six call sites are untouched. ⛔ Relabels ONLY the exact frozen default and ONLY on a genuinely empty window — **a measured zero keeps the frozen wording, because then it is TRUE.** ⚠ **DOC CORRECTION:** `FII_DII_FLOW` is recorded as one of three factors that *never score* — true of the 487-panel BACKTEST (empty table), **false live**, where it scores −0.2 on 22 of 52. The factor was not dead; its data was |
