...
|---|--:|--:|--:|--:|--:|--:|
| would-BLOCK | 407 | 59 | ₹-303 | ₹72 | ₹190 | 51% |
| eligible (kept) | 145 | 33 | ₹-7 | ₹-59 | ₹346 | 45% |
...
- 🚫 `tail` — would-block MEDIAN is ₹72 (positive) while its mean is ₹-303 — the negative mean is carried by a few large losses, so the gate would suppress a cohort that is TYPICALLY profitable
- 🚫 `win_rate` — would-block wins MORE often than eligible (51% vs 45%) — the gate is suppressing the higher-win-rate cohort

- **eligible set (the book a flip would leave you holding) — deflated Sharpe bar:** ⏳ does NOT clear — observed Sharpe -0.004 does not exceed the 20-trial benchmark +0.331 — more data cannot rescue it; the candidate is not ahead
